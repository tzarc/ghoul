...
repofind() {
    find . \( "$@" \) -and -not -path './.git/*' -print | sort
}
...
repofind -iname '*.sh' | parallel "chmod +x '{1}'"

# Convert all KiCad files to be in Unix format
repofind -type f -and \( -iname '*.kicad_*' -or -name 'fp-lib-table' -or -name 'sym-lib-table' -or -iname '*.md' -or -iname '*.svg' \) | parallel "dos2unix '{1}'"
